...
import { addJob, editJob, deleteJob, Job } from '../lib/jobsSlice';
import { addProject, editProject, deleteProject, Project } from '../lib/projectsSlice';
...
const Admin: React.FC = () => {
  ...
  const handleJobSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (editingJobId) {
      dispatch(editJob({ ...jobForm, id: editingJobId } as Job));
      setEditingJobId(null);
    } else {
      dispatch(addJob({ ...jobForm, id: uuidv4() } as Job));
    }
    setJobForm({});
  };
  ...
  const handleProjectSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (editingProjectId) {
      dispatch(editProject({ ...projectForm, id: editingProjectId } as Project));
      setEditingProjectId(null);
    } else {
      dispatch(addProject({ ...projectForm, id: uuidv4() } as Project));
    }
    setProjectForm({});
  };
